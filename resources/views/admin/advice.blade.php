...
<body class="bg-[#FBF4FA] text-gray-800 p-6">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach ($advices as $advice)
                ...
                <div id="editModal-{{ $advice->id }}" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
                    <form action="{{ route('advice.update', $advice->id) }}" method="POST" enctype="multipart/form-data"
                        ...
                        @csrf
                        @method('PUT')
                        <h2 class="text-2xl font-semibold text-[#C447AF] mb-4">Modifier le conseil</h2>
                        <div class="mb-4">
    ...
    <div id="createModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <form id="createForm" action="{{ route('createAdvice') }}" method="POST" enctype="multipart/form-data"
            class="bg-white p-6 rounded-2xl shadow-lg w-full max-w-lg">
            @csrf
            <h2 class="text-2xl font-semibold text-[#C447AF] mb-4">Ajouter un conseil</h2>
            <div class="mb-4">
                <label class="block mb-1 font-medium">Title</label>
